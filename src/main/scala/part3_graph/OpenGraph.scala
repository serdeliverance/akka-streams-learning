...
//  firstSouce.to(sinkGraph).run()
  ...
  val flowGraph = Flow.fromGraph(
    GraphDSL.create() { implicit builder => // remeber: builders are mutable
      import GraphDSL.Implicits._

      // all the operations that happend inside a builder operates on shapes
...
//  firstSouce.via(flowGraph).to(Sink.foreach(println)).run()
}
