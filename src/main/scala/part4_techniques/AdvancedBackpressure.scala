...
//  eventSource.to(notificationSink).run()
  ...
  /*
    Problem: slow consumer + un-backpressurable source

    Solution: when backpressure is need, instead of buffering in the component, we aggregate (conflate in akka... which is a fold operation)
    the incomming elements until the consumer is ready to receive more elements.
   */
  def sendEmailSlow(notification: Notification) = {
    Thread.sleep(1000)
    println(s"Dear ${notification.email} you have an event: ${notification.pagerEvent}")  // actually sened an email
  }
...
//    .to(Sink.foreach[Notification](sendEmailSlow)).run()  // alternative to backpressure

  /*
    Problem: fast consumer and slow producer

    Solution: extrapolate/expand
    It consists in creating a component between the source and the sink, which will be in charge of extrapolate elements
    to attend the consumer's demand. In other words... this bridge component produces elements to satisfy the consumer demand.
   */
