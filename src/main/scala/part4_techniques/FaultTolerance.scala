...
//    .run()
  ...
  // using this method, the stream stops after returning the value, but not crashes
...
//    .run()
  ...
  // after retry n times, it replaces the source with another source that you have provided
  faultySource.recoverWithRetries(3, {
    case _: RuntimeException => Source(90 to 99)
  }).log("recoverWithRetry")
    .to(Sink.ignore)
//    .run()
